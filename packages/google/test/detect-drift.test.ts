...
describe('detectDrift', () => {
  ...
  it("carries Google's edit stamp into the snapshot so the pinned pass can arbitrate titles", async () => {
    const store = fakeScheduledBlockStore([]);
    const client = new FakeGoogleClient();
    client.listQueue = [{
      events: [gEvent('g1', '2026-01-05T09:00:00.000Z', '2026-01-05T09:30:00.000Z', 'confirmed', '2026-01-05T08:30:00.000Z')],
    }];

    const result = await detectDrift({ client, scheduledBlocks: store }, 'u1', 'cal-auto', 'access', NOW, HORIZON_END);

    expect(result.observed.get('g1')!.updatedAt).toBe(Date.parse('2026-01-05T08:30:00.000Z'));
  });
  ...
  it('follows nextPageToken so blocks on later pages are neither deleted nor missed', async () => {
    // Google caps a listing page at 250 events. Reading only page 1 made every block
    // beyond it look deleted — and pinned rows now carry googleEventIds, so that is
    // user data loss, not just a churned auto-placement.
    const store = fakeScheduledBlockStore([
      makeScheduledBlock({ id: 'b1', googleEventId: 'g1', engineKey: 'task:t:0',
        startsAt: new Date('2026-01-05T09:00:00.000Z'), endsAt: new Date('2026-01-05T09:30:00.000Z') }),
      makeScheduledBlock({ id: 'b2', googleEventId: 'g2', engineKey: 'task:t:1',
        startsAt: new Date('2026-01-05T13:00:00.000Z'), endsAt: new Date('2026-01-05T13:30:00.000Z') }),
    ]);
    const client = new FakeGoogleClient();
    client.listQueue = [
      { events: [gEvent('g1', '2026-01-05T09:00:00.000Z', '2026-01-05T09:30:00.000Z')], nextPageToken: 'p2' },
      { events: [gEvent('g2', '2026-01-05T13:00:00.000Z', '2026-01-05T13:30:00.000Z')] },
    ];

    const result = await detectDrift({ client, scheduledBlocks: store }, 'u1', 'cal-auto', 'access', NOW, HORIZON_END);

    expect(result).toMatchObject({ pinned: 0, removed: 0 });
    expect(store.all().map((b) => b.id)).toEqual(['b1', 'b2']);
    expect(client.listCalls).toHaveLength(2);
    expect(client.listCalls[1]!.pageToken).toBe('p2');
    expect(result.observed.has('g2')).toBe(true);
  });
